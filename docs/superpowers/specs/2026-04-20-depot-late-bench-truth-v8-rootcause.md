...
| verifier 通过 | 116 (91.3%) | 117 (92.1%) | +1 |
| verifier 失败（solved=True, is_valid=False） | 1 | 0 | -1 |
| 容量预检拒绝 | 10 | 10 | +0 |
...
|---|---:|---:|---:|---:|
| depot_hook_count | 9.14 | 13 | 9.15 | 13 |
| depot_earliness | 201.79 | 406 | 189.38 | 371 |

**Earliness 总和**：OFF=23609，ON=22157，绝对下降 1452，相对下降 6.2% 。
...
- ON earliness 更小（大库更靠后）：**45** 场景
- 持平：**71** 场景
- ON earliness 更大（大库更靠前）：**0** 场景
- 总 earliness 绝对下降：**1237**
...
|---|---:|---:|---:|---|---|
| `validation_20260310W.json` | 62 | 40 | **-22** | constructive_partial | weighted_very_greedy |
...
| `validation_20260228W.json` | 10 | 104 | 48 | **-56** | beam | weighted |
...
- OFF 均值：0.370，ON 均值：0.419，理论上限均值：0.839
- ON 相对 OFF 向理论上限的空间收敛度：均值 12.1%（0% = 没挪，100% = 达到理论上限）
  - p25/p50/p75: 0.0% / 0.0% / 16.0%

## 8. 结论

- 配对样本 117 个场景，主目标总勾数 OFF=3773、ON=3749，变化 -24（净改善或持平）。
- **没有任何场景的勾数因开启而增加**，mitigation 生效。
- Earliness 总和 OFF=23609、ON=22157，变化 -1452。
