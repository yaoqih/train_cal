...
def reorder_depot_late(
    ...
    Worst case O(N^3): up to N passes x N adjacent checks x O(N) replay per
    check. For plan lengths typical to the depot-late use case (N <= 50)
    this is acceptable; the caller budgets this before verification.
    """
    result = list(plan)
    if not result:
        return result
